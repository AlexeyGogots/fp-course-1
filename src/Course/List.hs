...
length ::
  List a
  -> Int
length = foldLeft (\accum _ -> accum + 1) 0
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f = foldRight (\a as -> f a ++ as ) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
-- seqOptional = foldRight (\ox oxs -> bindOptional (\xs -> mapOptional (\x -> x :. xs) ox ) oxs) $ Full Nil
seqOptional = 
  let foo ox oxs = case (ox, oxs) of
                   (Full x, Full xs) -> Full (x :. xs)
                   (_, _) -> Empty
  in foldRight foo (Full Nil)
...
find ::
  ...
  -> List a
  -> Optional a
find p xs = foldRight (\a oa -> if p a then Full a else oa ) Empty xs
